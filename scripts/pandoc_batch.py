...
HRX = re.compile(r'(?m)^(?:-{3}|(?:\*\s*){3}|(?:_\s*){3})\s*\n(?!\s*\n)')

def patch_markdown_text(text: str) -> str:
    """Fix common Pandoc pitfalls:
    - Ensure a blank line after thematic breaks (---, ***, ___)
    ...
    # into:
    #   ---\n\nSome text
    text = HRX.sub(lambda m: m.group(0) + "\n", text)
    return text
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fix-inplace", action="store_true",
                   help="Write patched markdown back to source files (use with care).")
...
def run_one(infile: Path, outfile: Path | None, args: argparse.Namespace):
    # Ensure parent dir exists if we’re writing a file
